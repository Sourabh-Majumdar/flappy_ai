...
def main():
    ...
    alpha = 0.3
    POP_SIZE = 100
    Pc = 0              # Probability of Crossover
    Pm = 0.005          # Probability of Mutation
    generations = 500  # No of generations
    Model = list()
    best_individual  = Genome()
    best_score = 0          # Initially the best score is 0
    ...
    for g in range(generations) :          # The iteration for each generation
        ...
        if best_score < maximum_fitness :
            best_score = maximum_fitness

        average_fitness_list = np.append(average_fitness_list,average_fitness)
        maximum_fitness_list = np.append(maximum_fitness_list,best_score)
    ...
    f_handle = open('max_ga.txt','w')

    np.save('max_ga.npy',maximum_fitness_list)

    f_handle.close()

    f_handle = open('avg_ga.txt','w')

    np.save('avg_ga.npy',average_fitness_list)
    ...
    red_patch = mpatches.Patch(color = 'red',label = 'maximum fitness')
